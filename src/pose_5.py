...
def minimize_marginals(graph, initial_estimate, pose_options):
    ...
    sum_of_marginals = marginals.marginalCovariance(L(best_landmark)).sum() + marginals.marginalCovariance(L(2)).sum()
...
def minimize_errors(graph, initial_estimate, pose_options):
    ...
    list_of_errors = []
    for i in range(1,4):
        optimPos = result.atPose2(X(i))
        realPos = gtsam.Pose2(true_positions[i])
        dx = abs(optimPos.x() - realPos.x())
        dy = abs(optimPos.y() - realPos.y())
        dtheta = abs(optimPos.theta() - realPos.theta())
        posErr = dx + dy + dtheta
        list_of_errors.append(posErr)
